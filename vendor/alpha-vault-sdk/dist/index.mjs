...
import { AnchorProvider as AnchorProvider2, BN as BN3 } from "@coral-xyz/anchor";
import {
...
var fillDammV2Transaction = async (program, vaultKey, vault, payer) => {
  ...
  const [poolAuthority] = PublicKey2.findProgramAddressSync(
    [Buffer.from("pool_authority")],
    cpAmm._program.programId
  );
  const [dammEventAuthority] = PublicKey2.findProgramAddressSync(
...
var fillDlmmTransaction = async (program, vaultKey, vault, payer, opt) => {
  ...
  if (remainingInAmount.lte(new BN(0)))
    return;
  ...
  const dlmmProgramId = new PublicKey2(LBCLMM_PROGRAM_IDS[cluster]);
...
var BalanceTree = class {
  ...
  static toNode(account, maxCap) {
    const buf = Buffer.concat([
      account.toBuffer(),
      new BN2(maxCap).toArrayLike(Buffer, "le", 8)
    ]);
    const hashedBuff = Buffer.from(sha2562(buf), "hex");
    const bufWithPrefix = Buffer.concat([Buffer.from([0]), hashedBuff]);
    return Buffer.from(sha2562(bufWithPrefix), "hex");
  }
  getHexRoot() {
...
var AlphaVault = class {
  ...
  get vaultPoint() {
    ...
    const lastJoinPoint = Number(
      this.activationPoint.sub(
        process.env.NODE_ENV === "test" ? new BN3(5) : this.preActivationDuration
      ).sub(
        process.env.NODE_ENV === "test" ? new BN3(1) : new BN3(
          this.vault.activationType === 0 /* SLOT */ ? 750 : 5 * 60
        )
      ).toString()
    );
    const lastBuyingPoint = Number(
      this.activationPoint.sub(new BN3(1)).toString()
    );
  ...
  static async create(connection, vaultAddress, opt) {
    ...
    if (vault.poolType === 1 /* DAMM */) {
      ...
      return new AlphaVault(
        ...
        pool.bootstrapping.activationType === 0 /* SLOT */ ? new BN3(9e3) : new BN3(3600),
        clockState,
        baseMintInfo,
        quoteMintInfo,
        opt
    ...
    if (vault.poolType === 2 /* DAMMV2 */) {
      ...
      return new AlphaVault(
        ...
        vault,
        pool.activationPoint,
        pool.activationType === 0 /* SLOT */ ? new BN3(9e3) : new BN3(3600),
        clockState,
        baseMintInfo,
        quoteMintInfo,
        opt
  ...
  getClaimInfo(escrowAccount) {
    if (!escrowAccount || this.vault.totalDeposit.lten(0) || this.vault.boughtToken.lten(0)) {
      return {
        totalAllocated: new BN3(0),
        totalClaimed: new BN3(0),
        totalClaimable: new BN3(0)
      };
    }
    const currentSlot = this.clock.slot.toNumber();
    const currentTimestamp = this.clock.unixTimestamp.toNumber();
    ...
    const totalClaimable = (() => {
      const currentPoint = new BN3(
        this.vault.activationType === 0 /* SLOT */ ? currentSlot : currentTimestamp
      );
      if (currentPoint.lt(this.vault.startVestingPoint)) {
        return new BN3(0);
      }
      const endPoint = BN3.min(currentPoint, this.vault.endVestingPoint);
      const totalClaimableToken = this.vault.boughtToken.mul(endPoint.add(new BN3(1)).sub(this.vault.startVestingPoint)).div(
        this.vault.endVestingPoint.add(new BN3(1)).sub(this.vault.startVestingPoint)
      );
  ...
  getAvailableDepositQuota(escrow, merkleProof) {
    const deposited = escrow?.totalDeposit ?? new BN3(0);
    let remainingQuota = new BN3(Number.MAX_SAFE_INTEGER);
    if (this.vault.whitelistMode == 0 /* Permissionless */) {
      ...
      remainingQuota = merkleProof ? merkleProof.maxCap.sub(deposited) : new BN3(0);
    } else if (this.vault.whitelistMode == 2 /* PermissionWithAuthority */) {
      remainingQuota = escrow ? escrow.maxCap.sub(deposited) : new BN3(0);
    }
    let vaultCap = new BN3(Number.MAX_SAFE_INTEGER);
    if (this.mode === 1 /* FCFS */) {
      vaultCap = this.vault.maxDepositingCap.sub(this.vault.totalDeposit);
    }
    return BN3.min(remainingQuota, vaultCap);
  }
  ...
  canWithdrawDepositOverflow(escrow) {
    ...
    return currentPoint > this.vaultPoint.lastJoinPoint && currentPoint <= this.vaultPoint.lastBuyingPoint && escrowAvailableDepositOverflowAmount.gt(new BN3(0));
  }
  escrowAvailableDepositOverflowAmount(escrow) {
    ...
    ) ? this.vault.totalDeposit.sub(this.vault.maxBuyingCap) : new BN3(0);
  ...
  canWithdrawRemainingQuote(escrow) {
    ...
    return currentPoint > this.vaultPoint.lastBuyingPoint && remainingQuoteAmount.gt(new BN3(0)) && escrow.refunded === 0;
  }
  ...
  async getMerkleProofForDeposit(owner) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          merkleRootConfig: new PublicKey3(data.merkle_root_config),
          maxCap: new BN3(data.max_cap),
          proof: data.proof
        };
      }
    } catch (e) {
  ...
  getDepositInfo(escrowAccount) {
    ...
    const totalReturned = hasCrankDurationFinished ? remainingAmount.mul(escrowAccount.totalDeposit).div(this.vault.totalDeposit) : new BN3(0);
    const totalFilled = hasCrankDurationFinished ? escrowAccount.totalDeposit.sub(totalReturned) : new BN3(0);
    return {
